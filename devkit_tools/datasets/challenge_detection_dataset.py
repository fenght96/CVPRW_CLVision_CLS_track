...
class ChallengeDetectionDatasetv1(ChallengeDetectionDataset):
    """
    The sample dataset. For internal use by challenge organizers only.
    """
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(root=root,train=train,transform=transform,loader=loader,
                        ego_api=ego_api,img_ids=img_ids,bbox_format=bbox_format,
                        categories_id_mapping=categories_id_mapping)
        ...
        # json_file =  str(Path(root) / ann_json_path)
        # with open(json_file) as f:
        #     annos = json.load(f)
        # self.id_mapping = []

        # for i, cats in enumerate(annos['categories']):
        #     if cats['id'] not in self.id_mapping:
        #         self.id_mapping.append(cats['id'])

        if self.train:
            self.mix_data_index = []
            self.catid = [12, 15, 23, 41, 89, 93, 111, 121, 138, 170, 211, 221, 223, 240, 242, 270, 271]
            # self.ids_maped = [self.id_mapping.index(i) for i in self.catid]
            print(f'Use mix up\t train:{self.train}')
    ...
    def __getitem__(self, index):
        ...
        img, target = self._pre_img_labels(index)
        
        if self.train:
            if len(self.mix_data_index) > 1 and torch.rand(1) > 0.5:
                n = len(self.mix_data_index)
                idx = min(int(torch.rand(1) * n), n-1)
                idx_data = self.mix_data_index[idx]
                mix_img, mix_target = self._pre_img_labels(idx_data)
                h,w =  img.height , img.width  
                mix_h, mix_w = mix_img.height , mix_img.width  
                out_h, min_h = max(mix_h, h), min(mix_h, h)
                out_w, min_w = max(mix_w, w), min(mix_w, w)
                img_np = np.zeros([out_h, out_w, 3])
                img = np.asarray(img)
                mix_img = np.asarray(mix_img)
                img_np[:h, :w, :] = img
                img_np[:mix_h, :mix_w, :] = mix_img
                img_np[:min_h, :min_w, :] /= 2
                
                img = Image.fromarray(img_np.astype('uint8')).convert('RGB')
                del img_np
                del mix_img
                target["boxes"] = torch.cat([target["boxes"], mix_target["boxes"]], 0)
                target["labels"] = torch.cat([target["labels"], mix_target["labels"]], 0)
                target["area"] = torch.cat([target["area"], mix_target["area"]], 0)
                target["iscrowd"] = torch.cat([target["iscrowd"], mix_target["iscrowd"]], 0)
    ...
    def _pre_img_labels(self, index):
        img_id = self.img_ids[index]
        img_dict: EgoObjectsImage = self.ego_api.load_imgs(ids=[img_id])[0]
        annotation_dicts = self.targets[index]
        ...
        for i in range(num_objs):
            ...
            boxes.append([xmin, ymin, xmax, ymax])
            # ann_id = self.id_mapping.index(annotation_dicts[i]['category_id'])
            labels.append(annotation_dicts[i]['category_id'])
            
            if self.train and annotation_dicts[i]['category_id'] in self.catid:
                self.mix_data_index.append(index)
        ...
        if self.transform is not None:
            img, target = self.transform(img, target)
        if self.train:
            self.mix_data_index = list(set(self.mix_data_index))
