...
from devkit_tools.benchmarks import challenge_instance_detection_benchmark,challenge_category_detection_benchmark
...
DATASET_PATH = '/media/datd/fht0/cvprw/datasets/'
...
def main(args):
    ...
    model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model) # bn
    ...
    replay = ReplayMDetPlugin(mem_size=1200,task_balanced_dataloader=True)#, storage_policy = DetClassBalancedBuffer(1200))
    ewc = MEWCDetPlugin(ewc_lambda=0.001)
    save_checkpoint = ModelMCheckpoint(out_folder='./multi_detection_results', file_prefix='track2_output')
    mandatory_plugins = [replay, ewc, save_checkpoint]
    ...
    for experience in benchmark.train_stream:
        ...
        if False:#'valid' in benchmark.streams:
