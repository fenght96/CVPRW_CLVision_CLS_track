...
class ReplayDetDataLoader:
    ...
    def __init__(self, data: AvalancheDataset, memory: AvalancheDataset = None,
        ...
        max_len = max([len(d) for d in self.loader_data.values()])
        max_len_memory = max([len(d) for d in self.loader_memory.values()])
        self.max_len = max(max_len, max_len_memory // (self.batch_size - 1))
    ...
    def __iter__(self):
        ...
        max_len = max([len(d) for d in iter_data_dataloaders.values()])
        max_buffer_len = max([len(d) for d in iter_buffer_dataloaders.values()])
        max_len = max(max_len // (self.batch_size - 1), max_buffer_len)
