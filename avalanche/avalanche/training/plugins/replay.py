...
class ReplayDetPlugin(ReplayPlugin):
    """
    Experience replay plugin.
    ...
    def __init__(self, mem_size: int = 200, batch_size: int = None,
                 ...
                 task_balanced_dataloader: bool = False,
                 storage_policy: Optional["ExemplarsBuffer"] = None):
        super().__init__(mem_size, batch_size,batch_size_mem, task_balanced_dataloader, storage_policy)
        self.ori_dataloader = None
        self.flag = True
...
class ReplayMDetPlugin(ReplayDetPlugin):
    """
    Experience replay plugin.
    ...
    def before_training_exp(self, strategy: "SupervisedTemplate",
        ...
        sampler = torch.utils.data.distributed.DistributedSampler(strategy.adapted_dataset, rank=local_rank)
        strategy.dataloader = ReplayDetDataLoader(
            strategy.adapted_dataset,
            self.storage_policy.buffer,
            oversample_small_tasks=True,
