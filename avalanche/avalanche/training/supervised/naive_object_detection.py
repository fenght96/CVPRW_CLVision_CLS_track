...
class MultiObjectDetectionTemplate(ObjectDetectionTemplate):
    ...
    def make_train_dataloader(
        ...
        sampler = torch.utils.data.distributed.DistributedSampler(self.adapted_dataset, rank=local_rank)
        other_dataloader_args['sampler'] = sampler
        self.dataloader = TaskBalancedDataLoader(
            self.adapted_dataset,
            oversample_small_groups=True,
            num_workers=num_workers,
    ...
    def forward(self):
        ...
        if self.is_training:
            ...
            for i,t in enumerate(outs):
                t['boxes'] = t['boxes'] / self.mb_y[i]['scale_factor'].unsqueeze(0)
            return [{k: v.to('cpu') for k, v in t.items()} for t in outs]
